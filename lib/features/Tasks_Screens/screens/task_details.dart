...
class TaskDetailsScreen extends StatefulWidget {
 ...
 final List<dynamic>? file;
  ...
  const TaskDetailsScreen({super.key,
    ...
    required this.nameTask, required this.description,
    required this.deadline, required this.nameClient,
    required this.notes, required this.phoneClient, required this.address, required this.link, required this.id, required this.task_status,  required this.file});
          ...
          child: Column(
            children: [
              ...
              CustomAppBar(
                ...
                back: true,
                actions: [
                  role == '3' ? Container() :  PopupMenuButton<String>(
              ...
              TaskDetailsSection(nameEmployee: widget.nameEmployee ,nameTask: widget.nameTask ,
                description: widget.description ,deadline: widget.deadline,),
              SizedBox(height: 16.h),
              LocationSection(address: widget.address ,link: widget.link,),
              SizedBox(height: 16.h),
              ClientSection(nameClient: widget.nameClient,notes:widget.notes ,
                phoneClient: widget.phoneClient ,),
              SizedBox(height: 20.h),
            role == '3' ? (widget.task_status == 'completed' ? Container()
                :  Padding(
                padding:  EdgeInsets.symmetric(horizontal: AppDefaults.padding.w / 6 ),
                child: defaultButton(
                    context: context,
                    text: 'إكمال المهمة', width: 160.w,
                    height: 48.h, isColor: true, textSize: 18.sp, toPage: ()
                {
                  ...
                  CompleteTask(
                    ...
                    locationId: widget.locationId != 'لا يوجد' || widget.locationId != '' ? widget.locationId.toString() : '10'  ,
                    notes: widget.notes,
                ...
                : Container() ,
              widget.task_status != 'completed' ?   Row(
                    ...
                    onPressed: ()
                    {
                      _pickImage().then((onValue) async {
                        if (selectedImage != null) {
                          filesId =
                          await TasksCubit.get(context).uploadFileInTasks(selectedImage!);
             ...
             widget.file!.isEmpty  ?
             Container()
                 : AttachmentsSection(files: widget.file,),
              SizedBox(height: 20.h),
            ],
          ),
        ),
