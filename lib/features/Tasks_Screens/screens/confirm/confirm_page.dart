...
class _ConfirmTaskListPageState extends State<ConfirmTaskListPage> {
 ...
 Padding(
   padding:  EdgeInsets.symmetric(horizontal: AppDefaults.padding.w / 2),
   child: const CustomAppBar(back: false, title: 'المهمات الواردة'),
 ),
              BlocConsumer<TasksCubit,TasksStates>(builder: (context, state) {
                ...
                ListView.builder(
                  ...
                  shrinkWrap: true,
                  itemCount: TasksCubit.get(context).getUserTaskListWithStatus!.length,
                  itemBuilder: (context, index) =>
                      ConfirmTaskCardList(
                        statusColor: Colors.orange,
                        index: index,
                        id : TasksCubit.get(context).getUserTaskListWithStatus![index].id!.toInt(),
                        statusText: TasksCubit.get(context).getUserTaskListWithStatus![index].task_status.toString(),
                        taskName: TasksCubit.get(context).getUserTaskListWithStatus![index].title.toString(),
                        taskNotes: TasksCubit.get(context).getUserTaskListWithStatus![index].notes.toString(),
                        date: TasksCubit.get(context).getUserTaskListWithStatus![index].created_on.toString().substring(0,10),
                        textDate: 'تاريخ الانشاء' ,
                        location: TasksCubit.get(context).getUserTaskListWithStatus![index].location != null ?
